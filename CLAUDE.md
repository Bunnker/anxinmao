...
- **支持多猫切换 / 添加 / 删除**(2026-06-16 红线反转,原「不做多猫切换 UI」作废):切换条在
  `/pets`,数据 `Store.cats[] + activeCatId`;删猫连带删其 records,删空回新建流程。决策见
  `docs/superpowers/specs/2026-06-16-pets-page-multicat-design.md`(Q9)。
...
"iOS 克制精致 + 暖奶白底" —— 2026-06 由 Maple 分支整体重设计上线。
干净白卡、大圆角、克制留白、暖陶土红点缀(`--accent:#b05a50`)、暖奶白底
(`--paper:#f7f6f3` / `--surface:#ffffff` / `--ink:#1a1a18`);底部毛玻璃 Tab Bar
导航(首页 / 分诊 / 问答 / 我的,见 `src/components/TabBar.tsx`)。衬线只用于
标题 / 情绪文字,功能文字用黑体;中文不用斜体。

> 这是**视觉审美**(不是平台限定):移动端 H5,安卓 / iOS / 桌面任意浏览器一致访问。
> 风险三色(红 / 黄 / 绿)仍是**独立信号层**,不受品牌盘影响、且比正文更饱和
> (`--red:#d92d20 --amber:#b97900 --green:#238146`)—— 见上「产品红线」。

旧方向「温暖的纸质年鉴」(暖米纸色 + 衬线年鉴感)规格存档于
`docs/product/前端设计PRD-v0.1.md`,作为情绪基调参考。
...
- **`/`** —— 首页:greeting + 猫资料 + 两个主入口(分诊 / 行为问答)+ 安心知识入口
  + 最近记录
- **`/pets`** —— 毛孩子档案展示页(多猫切换条 + hero + 四宫格 + 生活相册照片墙[可单独编辑]
  + 健康档案[中性徽章] + 体重曲线 + 健康背景 + 健康足迹 + 健康记录 timeline[点回报告/对话])。
  TabBar「毛孩子」入口指向此页
- **`/onboarding`** —— 猫咪档案 编辑/添加表单(全屏,无 TabBar):`?pet=<id>` 编辑、`?add=1`
  添加、无参首次建档;含 AI 头像生成 + NOT_A_CAT 守门、疫苗逐针、相册、健康提醒开关、移除猫
- **`/symptoms`** —— 17 张症状卡(吐 / 拉 / 不吃 / 萎靡 / 打喷嚏 / 耳朵 / 皮肤 / 眼 /
  口腔 / 行为 / 跛行 / 误食 / 呼吸困难 / 大量出血 / 尿闭 / 小便不对劲 / 其它)
- **`/triage`** —— 症状分诊流程,16 个症状专属轨道(`src/lib/triage.ts` 的 `FLOWS`)
...
4. ~~**逐条经兽医审核**~~ —— 已完成(用户确认逐条审核已做):`ReviewedNotice` 现标
   ...
   - **2026-06-05 执业兽医面诊确认症状覆盖完整** —— 补「小便不对劲」(FLUTD/膀胱炎
     入口,复用尿闭 `uo` 证据,黄档尽快门诊、撞尿闭信号自动升红)后,共 **17 张症状卡 /
     16 轨道**,覆盖新手常见主诉。该卡内容已对 **Cornell FLUTD + iCatCare** 原文逐条核实
     (记录见 `docs/medical/source/cat-urethral-obstruction.source.md` §13)。
     → 当前信任背书三重:权威源逐条核对 + 执业兽医逐条审 + 面诊确认覆盖完整。
