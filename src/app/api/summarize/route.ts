...
export async function POST(req: Request): Promise<Response> {
  ...
  if (!messages || messages.length === 0) {
    return withCors(Response.json({ error: "没有要摘要的内容。" }, { status: 400 }), origin);
  }
