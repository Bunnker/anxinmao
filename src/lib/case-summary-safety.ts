...
const CERTAIN_DISEASE_RE = /猫瘟|支原体|疱疹|杯状|肾病|糖尿病|鼻炎|肺炎|口炎/;
const DIAGNOSIS_CERTAINTY_RE =
  /确诊|一定是|已经得了|百分百|肯定是|已经诊断(?:为|是)|就是.{0,12}(?:猫瘟|支原体|疱疹|杯状|肾病|糖尿病|鼻炎|肺炎|口炎)/;
...
const DOSAGE_UNIT_RE =
  /\d+(?:\.\d+)?\s*(?:mg|毫克|ml|毫升|片|粒|袋)|按体重.{0,8}(?:吃|用|喂)|连用.{0,6}天|疗程.{0,6}天|(?:剂量|用量).{0,8}(?:为|是|按|每次|每天|每日|一日|\d+(?:\.\d+)?\s*(?:mg|毫克|ml|毫升|片|粒|袋))/iu;
...
const CONCRETE_DOSAGE_RE =
  /\d+(?:\.\d+)?\s*(?:mg|毫克|ml|毫升|片|粒|袋)|(?:半|一|二|两|三|四|五|六|七|八|九|十)\s*(?:片|粒|袋|滴|ml|毫升|mg|毫克)|每次|每天|每日|一日|(?:每|每隔)\s*\d+\s*(?:小时|h)|早晚各一次|q\d+h|qxh|\b(?:qd|sid|tid|bid|qod)\b/iu;
const PRESCRIPTION_AMOUNT_RE =
  /每次\s*(?:半|一|二|两|三|四|五|六|七|八|九|十|\d+).{0,4}(?:片|粒|袋|滴|ml|毫升|mg|毫克)/iu;
const PRESCRIPTION_BARE_AMOUNT_RE =
  /(?:半|一|二|两|三|四|五|六|七|八|九|十|\d+(?:\.\d+)?)\s*(?:片|粒|袋|滴|ml|毫升|mg|毫克)/iu;
const PRESCRIPTION_DURATION_RE =
  /连用\s*(?:一|二|两|三|四|五|六|七|八|九|十|\d+)\s*(?:天|周|月)/iu;
const FREQUENCY_RE =
  /(?:一天|一日|每日|每天).{0,8}(?:\d+|[一二两三四五六七八九十]+)\s*次|(?:每|每隔)\s*\d+\s*(?:小时|h)(?:\s*(?:一|1)\s*次)?|早晚各一次|q\d+h|qxh|\b(?:qd|sid|tid|bid|qod)\b/iu;
const LINK_OR_SHOP_DRUG_PURCHASE_RE =
  /(?:链接|店铺).{0,8}(?:购买|买|下单).{0,8}(?:多西|阿奇|抗生素|消炎药|止痛药|止吐药|处方药|眼药水|耳药)/u;
const DRUG_PURCHASE_RE =
  /(?:购买|自行买|买点?|用药|喂药).{0,16}(?:多西|阿奇|抗生素|消炎药|止痛药|止吐药|处方药|眼药水|耳药)|(?:多西|阿奇|抗生素|消炎药|止痛药|止吐药|处方药|眼药水|耳药).{0,16}(?:购买|自行买|买点?|用药|喂药)|药店买|网上买|网上购买/;
...
function hasDrugPurchase(text: string): boolean {
  return text
    ...
    .some(
      (clause) =>
        LINK_OR_SHOP_DRUG_PURCHASE_RE.test(clause) ||
        MIXED_UNSAFE_DRUG_PURCHASE_RE.test(clause) ||
        !PROTECTIVE_DRUG_TEXT_RE.test(clause) && DRUG_PURCHASE_RE.test(clause),
    );
}
