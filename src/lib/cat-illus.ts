// 小猫表情 SVG kit —— 从 claude design 的 cat-illus.js 移植(纯字符串拼 SVG,零依赖)。
// 6 眼态 × 5 嘴态自由组合 + 12 个症状态 SYM(按症状卡标题取)+ 3 个情绪态 MOOD。
// 用途:头像、院子陪伴小图、报告卡角落情绪猫、症状卡装饰图标、问答等。viewBox 44×44。
// 红线:这是「头像/伴侣角色」用的卡通猫,SYM 是功能性症状示意图标,绝不与真实病征诊断信号绑定。
...
const NOSE = '<path d="M20.3 23.6 h3.4 l-1.7 1.8 z" fill="#c2756a"/>';

export const EYE = {
  open:
    '<circle cx="16" cy="21" r="2.9" fill="#4b3326"/><circle cx="28" cy="21" r="2.9" fill="#4b3326"/>' +
  ...
  happy:
    '<path d="M13.4 21.8 q2.6 -3.2 5.2 0 M25.4 21.8 q2.6 -3.2 5.2 0" stroke="#4b3326" stroke-width="1.9" fill="none" stroke-linecap="round"/>',
  worry:
    '<path d="M12.8 17.6 l4.6 1.7 M31.2 17.6 l-4.6 1.7" stroke="#4b3326" stroke-width="1.5" stroke-linecap="round"/>' +
  ...
  squeeze:
    '<path d="M13.4 20 l3.7 1.4 l-3.7 1.4 M30.6 20 l-3.7 1.4 l3.7 1.4" stroke="#4b3326" stroke-width="1.6" fill="none" stroke-linecap="round" stroke-linejoin="round"/>',
  droop:
    '<path d="M13.4 21.6 q2.6 2 5.2 0 M25.4 21.6 q2.6 2 5.2 0" stroke="#4b3326" stroke-width="1.8" fill="none" stroke-linecap="round"/>',
  side:
    '<circle cx="17.2" cy="21" r="2.9" fill="#4b3326"/><circle cx="29.2" cy="21" r="2.9" fill="#4b3326"/>' +
    '<circle cx="18.2" cy="20" r="1" fill="#fff"/><circle cx="30.2" cy="20" r="1" fill="#fff"/>',
} as const;

export const MOU = {
  smile:
    '<path d="M19.4 26.6 q2.6 2.4 5.2 0" stroke="#a06a55" stroke-width="1.5" fill="none" stroke-linecap="round"/>',
  tiny:
    '<path d="M20.4 26.4 q1.6 1.3 3.2 0" stroke="#a06a55" stroke-width="1.4" fill="none" stroke-linecap="round"/>',
  frown:
    '<path d="M19.4 27.2 q2.6 -2.2 5.2 0" stroke="#a06a55" stroke-width="1.5" fill="none" stroke-linecap="round"/>',
  pant:
    '<ellipse cx="22" cy="27" rx="2.2" ry="2.7" fill="#a85a4f"/><path d="M22 24.6 v2" stroke="#9c5248" stroke-width="1"/>',
...
export function cat(o: CatOpts = {}): string {
  return (
...
export const SYM: Record<string, string> = {
  ...
  眼睛问题: cat({
    eyes:
      '<circle cx="16" cy="21" r="2.5" fill="#4b3326"/><circle cx="28.4" cy="21.2" r="3.2" fill="#4b3326"/><circle cx="29.5" cy="20.1" r="1.15" fill="#fff"/><circle cx="17" cy="20.1" r=".9" fill="#fff"/>',
    mouth: MOU.frown,
    front:
