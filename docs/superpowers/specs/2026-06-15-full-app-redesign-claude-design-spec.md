...
> 版本 v0.1 · 2026-06-16 · 待用户审阅拍板
> 目标读者:实现者(你 / 后续 agent)。本文是「设计稿 → 现有 Next 产品」的落地总规格,逐页给映射与红线护栏。
> 设计稿目录:`/tmp/design-import/untitled/work/`(home/A/B/C/triage/inquiry/report/qa/pets/pet-edit + shared.css + cat-illus.js)
...
终稿 = `home.html`(A/B/C 是探索稿,仅作院子姿态参考)。

**区块顺序(自上而下):** 问候条 → 价值主张 → 看病 CTA 大按钮 → 「院子」section → 院子 hero → 「最近记录」section → record 卡 → TabBar。
